...
import 'package:delivery_app/src/models/mercado_pago_issuer.dart';
...
class MercadoPagoPaymentMethodInstallments {
  ...
  static List<MercadoPagoPaymentMethodInstallments> fromJsonList(
    List<dynamic> jsonList,
  ) {
    List<MercadoPagoPaymentMethodInstallments> toList = [];

    jsonList.forEach((item) {
      MercadoPagoPaymentMethodInstallments model =
          MercadoPagoPaymentMethodInstallments.fromJson(item);
      toList.add(model);
    });
