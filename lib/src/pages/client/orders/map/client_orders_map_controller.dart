...
class ClientOrdersMapController extends GetxController {
  ...
  void connectAndListen() {
    print('LOG() connectAndListen');
    socket.connect();
    socket.onConnect((data) {
      print('CLIENT ORDERS MAP : ESTE DISPOSITIVO SE CONECTO A SOCKET IO');
    });
    listenPosition();
    listenToDelivered();
  }
  ...
  void listenPosition() {
    socket.on('position/${order.id}', (data) {
      try {
        ...
        LatLng fromDeliveryPosition = LatLng(data['lat'], data['lng']);

        LatLng toClientPosition = LatLng(
          order.address?.lat ?? -11.976272,
          order.address?.lng ?? -76.9169402,
        );

        setPolylines(fromDeliveryPosition, toClientPosition);
      } catch (e) {
        print('Error al procesar la posición del repartidor: $e');
      }
    });
  }
    ...
    socket.on('delivered/${order.id}', (data) {
      try {
        ...
        Fluttertoast.showToast(
          msg: 'El estado de tu orden se actualizo',
          toastLength: Toast.LENGTH_LONG,
        );
      } catch (e) {
        print('Error al listenToDelivered: $e');
      }
    });
  }
  ...
  void updateLocation() async {
    try {
      ...
      saveLocation();
      animateCameraPosition(order.lat ?? -11.976272, order.lng ?? -76.9169402);

      addMarker(
        'delivery',
        order.lat ?? -11.976272,
        order.lng ?? -76.9169402,
        'Tu repartidor',
        '',
        deliveryMarker!,
      );
      ...
      LatLng from = LatLng(order.lat ?? -11.976272, order.lng ?? -76.9169402);
      LatLng to = LatLng(
        order.address?.lat ?? -11.976272,
        order.address?.lng ?? -76.9169402,
      );
